...
/**
 * Repository for managing volunteer event registrations.
 * Supports queries for registration status, volunteer history,
 * organizer statistics, and advanced filtering with status conditions.
 */
public interface RegistrationRepository extends JpaRepository<RegistrationEntity, Long> {
       ...
       long countByVolunteerAndStatusIn(UserEntity volunteer,
                     Collection<RegistrationEntity.RegistrationStatus> statuses);

       long countByEventOrganizer(UserEntity organizer);

       long countByEventOrganizerAndStatusIn(UserEntity organizer,
                     Collection<RegistrationEntity.RegistrationStatus> statuses);
       ...
       List<RegistrationEntity> findByEventAndStatusIn(EventEntity event,
                     Collection<RegistrationEntity.RegistrationStatus> statuses);
       ...
       Optional<RegistrationEntity> findByEventIdAndVolunteerIdWithDetails(@Param("eventId") Long eventId,
                     @Param("volunteerId") Long volunteerId);

       @Query("SELECT r FROM RegistrationEntity r " +
                     "WHERE r.event.id = :eventId AND r.volunteer.id = :volunteerId")
       Optional<RegistrationEntity> findByEventIdAndVolunteerIdSimple(@Param("eventId") Long eventId,
                     @Param("volunteerId") Long volunteerId);
}
